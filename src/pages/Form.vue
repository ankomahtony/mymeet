...
<form @submit.prevent="payViaService" class="min-w-screen min-h-screen bg-gray-900 flex items-center justify-center px-5 py-5" >
    
    <div class="bg-gray-100 text-gray-500 rounded-3xl shadow-xl w-full overflow-hidden" style="max-width:1000px">
        <div class="md:flex w-full">
            <div class="hidden md:block w-1/2 bg-indigo-500 py-10 px-10">
                <img width="100%" height="100%" src="https://img.icons8.com/cute-clipart/512/000000/share.png"/>
            ...
            <div class="w-full md:w-1/2 py-10 px-5 md:px-10">
                <div class="text-center mb-10">
                    ...
                    <p class="text-3xl font-semibold text-indigo-700">{{meeting_name}}</p>
                </div>
                
                <div>
                <div class="flex -mx-3">
                    <div class="w-full px-3 mb-5">
                        <label class="text-xs font-semibold px-1 ">Name:</label><span class="text-red-400 font-bold">*</span> 
                        <div class="flex" v-if='info.name'>
                            <div class="w-10 z-10 pl-1 text-center pointer-events-none flex items-center justify-center">
                                <i class="mdi mdi-email-outline text-gray-400 text-lg"></i>
                            </div>
                            <input v-model="paymentData.customer.name" type="text"  placeholder="Your Name"   class="w-full -ml-10 pl-3 pr-3 py-2 rounded-lg border-2 border-gray-200 outline-none focus:border-indigo-500" autocomplete="off">
                        </div>
                ...
                <div class="flex -mx-3">
                    <div class="w-full px-3 mb-5">
                        <label class="text-xs font-semibold px-1 ">Email:</label><span class="text-red-400 font-bold">*</span> 
                        
                        <div class="flex" v-if='info.email'>
                            <div class="w-10 z-10 pl-1 text-center pointer-events-none flex items-center justify-center">
                                <i class="mdi mdi-email-outline text-gray-400 text-lg"></i>
                            </div>
                            <input v-model="paymentData.customer.email" type="email"  placeholder="Your Email"   class="w-full -ml-10 pl-3 pr-3 py-2 rounded-lg border-2 border-gray-200 outline-none focus:border-indigo-500" autocomplete="off">
                        </div>
                ...
                <div class="flex -mx-3">
                    <div class="w-full px-3 mb-5">
                        <label class="text-xs font-semibold px-1 ">Phone:</label><span class="text-red-400 font-bold">*</span> 
                        
                        <div class="flex" v-if='info.phone'>
                            <div class="w-10 z-10 pl-1 text-center pointer-events-none flex items-center justify-center">
                                <i class="mdi mdi-email-outline text-gray-400 text-lg"></i>
                            </div>
                            <input v-model="paymentData.customer.phone_number" type="tel"  placeholder="Your Phone Number"   class="w-full -ml-10 pl-3 pr-3 py-2 rounded-lg border-2 border-gray-200 outline-none focus:border-indigo-500" autocomplete="off">
                        </div>
                    ...
                    <input type="number" :value="info.meeting_id" hidden>
                    <input type="text" v-model="meeting_name" hidden>
                    <field v-for="field in info.fields" :key="field.id" :field="field" :answer="answer"  @changeAnswer="myNewAnswer($event)"/>
                    {{answer}}
...
export default {
    ...
    return {
      ...
      paymentData: {
        ...
        customizations: {
          ...
          description: 'Pay to be part of this meeting',
  ...
  mounted () {
    ...
    axios.get('/api/meetings/form/'+uuid)
      .then(response => (this.info = response.data, this.meeting_name=response.data.meeting_name))
      .catch(()=>{
  ...
  methods:{
      myNewAnswer(e){
          if(!this.answer.some(data => data.id === e.id)){
              this.answer.push(e)
          }
      },
    ...
    async sendData(){
        ...
        await axios.post('/api/responses',[this.answer, this.paymentData.customer.name, this.paymentData.customer.email, this.paymentData.customer.phone_number, this.info.meeting_id, this.paymentData.tx_ref]).then(()=>{
            this.$router.replace({ name: 'Congratulation' })
